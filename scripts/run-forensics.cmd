@echo off
setlocal
rem LEDGER AND PRICE-NAMESPACE FORENSICS. READ-ONLY.
...
echo.
echo ===============================================================
echo  LEDGER FORENSICS - us-pit-sample400 @ f78cfe45b962
echo   1. Release build, warnings are errors
echo   2. ledger and price-namespace trace, read-only
...
dotnet build "%~dp0..\AI-Investment-Analyst.sln" -c Release --nologo > "%~dp0..\artifacts\verify\forensics-build.log" 2>&1
set EXITCODE=%ERRORLEVEL%
if not "%EXITCODE%"=="0" (
  echo   FAIL  Release build
  findstr /C:"error " "%~dp0..\artifacts\verify\forensics-build.log"
  goto :done
)
echo   PASS  Release build
echo.

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-forensics.ps1"
set EXITCODE=%ERRORLEVEL%
