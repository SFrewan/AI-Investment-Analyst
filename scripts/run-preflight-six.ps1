...
<#
    CATEGORY E SIX-MEMBER LOCAL REPLAY PREFLIGHT. READ ONLY.
...
$tool = Join-Path $root 'tools\preflight-six\PreflightSix.csproj'
$log = Join-Path $root 'artifacts\verify\preflight-six-build.log'

$null = New-Item -ItemType Directory -Force -Path (Join-Path $root 'artifacts\verify')

Write-Host ''
Write-Host '  ================================================================'
Write-Host '   CATEGORY E SIX-MEMBER LOCAL REPLAY PREFLIGHT   (READ ONLY)'
...
$exe = Join-Path $root 'tools\preflight-six\bin\Release\net8.0\preflight-six.exe'
