...
echo.
echo ===============================================================
echo  SPLIT AUTHORISATION PREPARATION - NO DISPATCH
echo   1. Release build, warnings are errors
echo   2. install the successor authorisation, cut its batches
...
dotnet build "%~dp0..\AI-Investment-Analyst.sln" -c Release --nologo > "%~dp0..\artifacts\verify\splitauth-build.log" 2>&1
set EXITCODE=%ERRORLEVEL%
if not "%EXITCODE%"=="0" (
  echo   FAIL  Release build
  findstr /C:"error " "%~dp0..\artifacts\verify\splitauth-build.log"
  goto :done
)
echo   PASS  Release build
echo.

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-splitauth.ps1"
set EXITCODE=%ERRORLEVEL%
