...
<#
    PROVE THE SUBMISSIONS COMPLETENESS GATE. READ-ONLY. PURE LOGIC AND LITERALS.
    ...
    artifacts\verify\sec-edgar-completeness-gate.md
...
try {
    ...
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -Filter 'FullyQualifiedName~SubmissionsCompletenessTests' `
        -LogName 'completeness.log' `
        -Label 'the submissions completeness gate: an unproven history never reports an absence' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'completeness-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host ''
Write-Host '=== Completeness gate outcome'

$report = Join-Path $root 'artifacts\verify\sec-edgar-completeness-gate.md'
