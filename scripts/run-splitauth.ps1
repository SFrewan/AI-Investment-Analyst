...
try {
    $env:AIINV_SPLIT_AUTHORIZATION = '1'

    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~SplitAuthorizationTests' `
        -LogName 'split-authorization.log' `
        -Label 'split authorisation preparation, no dispatch' | Out-Host

    $code = [int]$LASTEXITCODE
}
finally { Remove-Item Env:\AIINV_SPLIT_AUTHORIZATION -ErrorAction SilentlyContinue }
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'splitauth-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host ''
Write-Host '=== Split authorisation outcome'

$report = Join-Path $root 'artifacts\verify\split-authorization.md'
...
else { Write-Host '  no split authorisation report written.' }

Write-Host ''
Write-Host '  NOTHING WAS ACQUIRED. No EODHD call, no SEC call, no price, no split, no dividend.'
Write-Host '  NO SPLIT BATCH RAN. EVERY SPLIT BATCH IS STILL MARKED UNAUTHORISED.'
