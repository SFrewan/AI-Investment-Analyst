...
<#
    POST-RUN VERIFICATION FOR SEC BATCH 4 (NGM). READ-ONLY, THEN THE SUITE.
    ...
    and lives in artifacts\verify\sec-ngm-evidence.json; it is re-read and re-checked here rather
...
Write-Host ''
Write-Host '  ================================================================'
Write-Host '   POST-RUN VERIFICATION - SEC BATCH 4 - NGM.US'
...
$evidence = Join-Path $root 'artifacts\verify\sec-ngm-evidence.json'

if (Test-Path $evidence) {
    $e = Get-Content $evidence -Raw | ConvertFrom-Json
    Write-Host ''
    Write-Host '=== the batch 4 census, re-read from the file the dispatch run wrote'
    ...
    Write-Host ''
    Write-Host '  STOPPING. No batch 4 evidence summary exists.'
    exit 2
}
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'ngm-door.log' -Label 'the execution door, at rest again' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'ngm-partition.log' -Label 'the partition and its installation' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'ngm-runner.log' -Label 'the runner invariants, unchanged' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'ngm-preflight.log' -Label 'the preflight gates, unchanged' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'ngm-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host '  NOTHING WAS CONSUMED. The authorisation stands at consumed 4, remaining 2.'
Write-Host '  BATCH 5 WAS NOT APPROVED AND DID NOT RUN.'
