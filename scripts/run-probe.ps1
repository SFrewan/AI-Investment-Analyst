...
$code = 0
$probeReport = Join-Path $root 'artifacts\verify\survivorship-probe-extended.md'
...
if (Test-Path $probeReport) {
    ...
    try {
        $env:AIINV_SURVIVORSHIP_PROBE = '1'

        & powershell -NoProfile -ExecutionPolicy Bypass `
            -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
            -Filter 'FullyQualifiedName~SurvivorshipProbeTests' `
            -LogName 'survivorship-probe.log' `
            -Label 'widened survivorship probe, at most 10 free SEC requests' | Out-Host
