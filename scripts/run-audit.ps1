...
<#
    PRE-ACQUISITION AUTHORISATION AUDIT. READ-ONLY.

    ZERO provider calls of any kind. Nothing is acquired: no prices, no splits, no dividends, no
    corporate actions, no scores, no orders. Nothing is written to the database, and the sealed
    manifest is neither modified nor resealed - its bytes and digest are compared against the
    approved values.
...
try {
    $env:AIINV_ACQUISITION_AUDIT = '1'

    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~AcquisitionAuthorizationTests' `
        -LogName 'acquisition-audit.log' `
        -Label 'pre-acquisition authorisation audit, read-only' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'audit-full.log' -Label 'full Release suite' | Out-Host
...
Write-Host ''
Write-Host '=== Authorisation audit outcome'

$report = Join-Path $root 'artifacts\verify\acquisition-authorization.md'

if (Test-Path $report) {
    Get-Content $report |
        Where-Object { $_ -match 'Bytes |SHA-256|Matches the approved|Enabled|ApiKey|MaxRequests|MaxActions|MaxCost|DataIngestion|FamilyBudget|Runs against|Dispatched|Distinct request|seam execution|Failed or refused|Quarantined|Acquisition-ready|Not ready|malformed|non-authoritative|but equity|shared by two|Planned requests|Distinct fingerprints|Already satisfied|Net fetches|Deterministic|wrong window|unexpected endpoint|disagreeing|Gate 2|Gate 6|Gate 12' } |
