@echo off
setlocal
rem POST-RUN VERIFICATION FOR SEC BATCH 4 (NGM)
...
echo.
echo ===============================================================
echo  POST-RUN VERIFICATION - SEC BATCH 4 - NGM.US
echo   ZERO SEC calls. ZERO EODHD calls. ZERO network requests.
echo ===============================================================
echo.

echo --- Release build (warnings are errors)
dotnet build "%~dp0..\AI-Investment-Analyst.sln" -c Release --nologo > "%~dp0..\artifacts\verify\ngm-verify-build.log" 2>&1
set EXITCODE=%ERRORLEVEL%
if not "%EXITCODE%"=="0" (
  echo   FAIL  Release build
  findstr /C:"error " "%~dp0..\artifacts\verify\ngm-verify-build.log"
  goto :done
)
echo   PASS  Release build
echo.

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-ngm-verify.ps1"
set EXITCODE=%ERRORLEVEL%
