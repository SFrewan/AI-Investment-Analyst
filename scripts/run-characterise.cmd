@echo off
setlocal
rem CHARACTERISE EVERY REFUSED SESSION PAIR AGAINST ARCHIVED OHLCV - READ-ONLY DIAGNOSTIC.
...
rem   One report is written: artifacts\verify\gate6-unexplained-move-characterisation.md

cd /d "%~dp0.."

echo.
echo ===============================================================
echo  UNEXPLAINED-MOVE CHARACTERISATION - READ-ONLY, NO ACQUISITION
echo   1. Release build, warnings are errors
echo   2. read archived OHLCV context around every refused session pair
...
dotnet build "%~dp0..\AI-Investment-Analyst.sln" -c Release --nologo > "%~dp0..\artifacts\verify\characterise-build.log" 2>&1
set EXITCODE=%ERRORLEVEL%
if not "%EXITCODE%"=="0" (
  echo   FAIL  Release build
  findstr /C:"error " "%~dp0..\artifacts\verify\characterise-build.log"
  goto :done
)
echo   PASS  Release build
echo.

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-characterise.ps1"
set EXITCODE=%ERRORLEVEL%
