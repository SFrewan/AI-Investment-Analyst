...
<#
    POST-RUN VERIFICATION FOR SEC BATCH 5 (SDC). READ-ONLY, THEN THE SUITE.
    ...
    and lives in artifacts\verify\sec-sdc-evidence.json; it is re-read and re-checked here rather
...
Write-Host ''
Write-Host '  ================================================================'
Write-Host '   POST-RUN VERIFICATION - SEC BATCH 5 - SDC.US'
...
$evidence = Join-Path $root 'artifacts\verify\sec-sdc-evidence.json'

if (Test-Path $evidence) {
    $e = Get-Content $evidence -Raw | ConvertFrom-Json
    Write-Host ''
    Write-Host '=== the batch 5 census, re-read from the file the dispatch run wrote'
    ...
    Write-Host ''
    Write-Host '  STOPPING. No batch 5 evidence summary exists.'
    exit 2
}
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'sdc-door.log' -Label 'the execution door, at rest again' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'sdc-partition.log' -Label 'the partition and its installation' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'sdc-runner.log' -Label 'the runner invariants, unchanged' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'sdc-preflight.log' -Label 'the preflight gates, unchanged' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'sdc-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host '  NOTHING WAS CONSUMED. The authorisation stands at consumed 5, remaining 1.'
Write-Host '  BATCH 6 WAS NOT APPROVED AND DID NOT RUN.'
