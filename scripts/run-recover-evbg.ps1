...
<#
    LOCAL ARCHIVED-PAYLOAD RECOVERY - EVBG.US ONLY.

    THIS SCRIPT WRITES.

    What it does NOT do: no EODHD call, no SEC call, no acquisition, no acquisition authorisation,
    no new ingestion run, no second replay, no retry, no CCF replay, no GPP / VLDR / WIRE replay.
    Gate 6, SplitAdjustment, the declarations and the sealed manifest are not touched.

    What it does: builds tools\replay-recover (net8.0) and runs it once for EVBG.US. The tool
    ...
    Original ingestion run: e2f77177-4827-43bf-8951-2f56a28c0f2e

    The symbol is passed explicitly and the tool refuses anything that is not one of the five.
    CCF cannot be replayed again by this path: its subject now holds observations and its
    idempotency key is claimed, so preconditions 12 and 13 would abort before any write.
...
$log = Join-Path $root 'artifacts\verify\recovery-evbg-build.log'

$null = New-Item -ItemType Directory -Force -Path (Join-Path $root 'artifacts\verify')

Write-Host ''
Write-Host '  ================================================================'
Write-Host '   ARCHIVED PRICE RECOVERY - EVBG.US ONLY'
...
Write-Host '   CCF is NOT replayed. GPP, VLDR and WIRE are NOT touched.'
Write-Host '  ================================================================'
Write-Host ''
...
Write-Host ''
Write-Host '  EVBG.US ONLY. CCF was not replayed again. GPP, VLDR and WIRE were not touched.'
Write-Host '  The five [] members were not touched.'
