...
<#
    ...
    SCOPE, and nothing wider:
      * eodhd-eod ONLY. No splits, no dividends, no SEC, no benchmark, no scoring.
...
if ($code -ne 0) {
    ...
    Write-Host ''
    Write-Host '  ================================================================'
    Write-Host ("   BATCH " + $Batch + ": SPENDING AT MOST 70 EODHD REQUESTS.")
    Write-Host '   eodhd-eod ONLY. No splits, no dividends, no SEC.'
    Write-Host '   Window 2021-09-01..2026-08-31. Ceiling 704, unamended.'
    ...
    try {
        ...
        $env:AIINV_ACQUISITION_BATCH = '1'
        $env:AIINV_BATCH_INDEX = [string]$Batch

        & powershell -NoProfile -ExecutionPolicy Bypass `
            -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
            -Filter 'FullyQualifiedName~AcquisitionBatchTests' `
            -LogName ('acquisition-batch-' + $Batch + '.log') `
            -Label ('batch ' + $Batch + ', at most 70 EODHD price requests') | Out-Host
...
try {
    ...
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'batch-post-acquisition.log' `
...
try {
    ...
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'batch-quarantine-survey.log' `
...
& powershell -NoProfile -ExecutionPolicy Bypass `
    ...
    -LogName 'batch-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
$files = @(
    (Join-Path $root ('artifacts\verify\acquisition-batch-' + $label + '.md')),
...
Write-Host ''
Write-Host '  NO FURTHER BATCH RAN. The next batch is marked unauthorised in the runner and'
Write-Host '  requires its own approval. No splits, no dividends and no SEC call were made.'
