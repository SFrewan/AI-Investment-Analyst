...
<#
    HOLD EVERY REFUSED SESSION PAIR AGAINST EVERY DATED THING HELD LOCALLY. READ-ONLY.
    ...
    artifacts\verify\gate6-local-coincidence-check.md
...
try {
    $env:AIINV_COINCIDENCE_CHECK = '1'

    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~LocalCoincidenceCheckTests' `
        -LogName 'coincidence.log' `
        -Label 'every refused session pair, against every dated thing the repository holds' | Out-Host

    $code = [int]$LASTEXITCODE
}
finally { Remove-Item Env:\AIINV_COINCIDENCE_CHECK -ErrorAction SilentlyContinue }

if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~AI.Investment' `
        -LogName 'coincidence-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host ''
Write-Host '=== Local coincidence outcome'

$report = Join-Path $root 'artifacts\verify\gate6-local-coincidence-check.md'
...
else { Write-Host '  no coincidence report written.' }
