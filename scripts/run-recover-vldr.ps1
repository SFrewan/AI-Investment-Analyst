...
<#
    LOCAL ARCHIVED-PAYLOAD RECOVERY - VLDR.US ONLY.

    THIS SCRIPT WRITES.

    What it does NOT do: no EODHD call, no SEC call, no acquisition, no acquisition authorisation,
    no new ingestion run, no second replay, no retry, no CCF / EVBG / GPP replay, no WIRE replay.
    Gate 6, SplitAdjustment, the declarations and the sealed manifest are not touched.

    What it does: builds tools\replay-recover (net8.0) and runs it once for VLDR.US. The tool
    ...
    Original ingestion run: e4097a2d-1db7-4c85-9388-6b45cad106c4

    The symbol is passed explicitly and the tool refuses anything that is not one of the five.
    CCF, EVBG and GPP cannot be replayed again by this path: their subjects now hold observations
    and their idempotency keys are claimed, so preconditions 12 and 13 would abort before any write.
...
$log = Join-Path $root 'artifacts\verify\recovery-vldr-build.log'

$null = New-Item -ItemType Directory -Force -Path (Join-Path $root 'artifacts\verify')

Write-Host ''
Write-Host '  ================================================================'
Write-Host '   ARCHIVED PRICE RECOVERY - VLDR.US ONLY'
...
Write-Host '   CCF, EVBG and GPP are NOT replayed. WIRE is NOT touched.'
Write-Host '  ================================================================'
Write-Host ''
...
Write-Host ''
Write-Host '  VLDR.US ONLY. CCF, EVBG and GPP were not replayed again.'
Write-Host '  WIRE was not touched. The five [] members were not touched.'
