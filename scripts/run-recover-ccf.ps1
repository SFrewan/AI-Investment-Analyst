...
<#
    LOCAL ARCHIVED-PAYLOAD RECOVERY - CCF.US ONLY.

    THIS SCRIPT WRITES. It is the first one in this sequence that does.

    What it does NOT do: no EODHD call, no SEC call, no acquisition, no acquisition authorisation,
    no new ingestion run, no second replay, no retry. Gate 6, SplitAdjustment, the declarations and
    the sealed manifest are not touched.

    What it does: builds tools\replay-recover (net8.0) and runs it once for CCF.US. The tool
    re-verifies every precondition against the live ai_investment database and STOPS before the
...
$log = Join-Path $root 'artifacts\verify\recovery-ccf-build.log'

$null = New-Item -ItemType Directory -Force -Path (Join-Path $root 'artifacts\verify')

Write-Host ''
Write-Host '  ================================================================'
Write-Host '   ARCHIVED PRICE RECOVERY - CCF.US ONLY'
...
Write-Host ''
Write-Host '  CCF.US ONLY. EVBG, GPP, VLDR and WIRE were not touched.'
