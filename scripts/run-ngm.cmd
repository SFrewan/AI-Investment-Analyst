@echo off
setlocal
rem SEC BATCH 4 - NGM.US - CIK 0001426332
rem   THIS STEP REACHES THE U.S. SECURITIES AND EXCHANGE COMMISSION.
rem   ONE company. ONE request. ONE authorisation unit, the fourth of six.
rem   submissions/CIK0001426332.json, category RegulatoryFilings, NO window on the
...
rem   EODHD stays off. Batches 1-3 and 5-6 do NOT run.
...
echo.
echo ===============================================================
echo  SEC BATCH 4 - NGM.US - CIK 0001426332
...
dotnet build "%~dp0..\AI-Investment-Analyst.sln" -c Release --nologo > "%~dp0..\artifacts\verify\sec-ngm-build.log" 2>&1
set EXITCODE=%ERRORLEVEL%
if not "%EXITCODE%"=="0" (
  echo   FAIL  Release build - nothing was spent
  findstr /C:"error " "%~dp0..\artifacts\verify\sec-ngm-build.log"
  goto :done
)
echo   PASS  Release build
echo.

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-ngm.ps1"
set EXITCODE=%ERRORLEVEL%
