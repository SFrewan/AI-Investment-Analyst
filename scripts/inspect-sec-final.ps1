...
Show 'SEC ingestion runs by outcome (all time)' (
    "select outcome, count(*) as runs from ingestion_runs " +
    "where request_source_id = 'sec-edgar' group by 1 order by 2 desc")

Show 'SEC ingestion runs, newest first' (
    "select started_at_utc, outcome, refusal_rule_id from ingestion_runs " +
    "where request_source_id = 'sec-edgar' order by started_at_utc desc limit 25")

Show 'quarantined payloads by rule (all time)' (
    'select source_id, category, rule_id, count(*) as payloads ' +
    'from quarantined_payloads group by 1, 2, 3 order by 4 desc')

Show 'quarantined payloads today' (
    'select count(*) as payloads_today from quarantined_payloads ' +
    "where quarantined_at_utc > now() - interval '1 day'")
...
Show 'action executions per capability, last hour' (
    'select capability, count(*) as actions ' +
    "from action_executions where started_at_utc > now() - interval '1 hour' " +
    'group by 1 order by 2 desc')

Show 'columns of action_executions' (
    "select column_name, data_type from information_schema.columns " +
    "where table_name = 'action_executions' order by ordinal_position")

Show 'columns of audit_records' (
    "select column_name, data_type from information_schema.columns " +
    "where table_name = 'audit_records' order by ordinal_position")

Show 'audit outcomes today' (
    'select action_type, outcome, count(*) as times from audit_records ' +
    "where occurred_at_utc > now() - interval '1 day' " +
    'group by 1, 2 order by 3 desc limit 30')

Show 'fundamental observations stored' (
    "select count(*) as observations, count(distinct subject_identifier) as companies, " +
    'count(distinct attribute) as attributes from observations ' +
    "where attribute like 'financials.%'")

Show 'fundamental observation period range' (
    'select min(as_of_utc) as earliest_period, max(as_of_utc) as latest_period, ' +
    'min(published_at_utc) as earliest_publication, max(published_at_utc) as latest_publication ' +
    "from observations where attribute like 'financials.%'")

Show 'point-in-time invariants (both must be 0)' (
    'select ' +
    'count(*) filter (where published_at_utc < as_of_utc) as published_before_period, ' +
    'count(*) filter (where published_at_utc > retrieved_at_utc) as published_after_retrieval ' +
    "from observations where attribute like 'financials.%'")

Show 'companies covered' (
    'select subject_identifier, count(*) as observations, ' +
    'count(distinct attribute) as attributes, min(as_of_utc) as earliest ' +
    "from observations where attribute like 'financials.%' " +
    'group by 1 order by 2 desc')

Show 'opportunities (must be unchanged)' 'select count(*) as opportunities from opportunities'
