...
<#
    POST-RUN VERIFICATION FOR SEC BATCH 6 (SHPW). READ-ONLY, THEN THE SUITE.
    ...
    and lives in artifacts\verify\sec-shpw-evidence.json; it is re-read and re-checked here rather
...
Write-Host ''
Write-Host '  ================================================================'
Write-Host '   POST-RUN VERIFICATION - SEC BATCH 6 - SHPW.US'
...
$evidence = Join-Path $root 'artifacts\verify\sec-shpw-evidence.json'

if (Test-Path $evidence) {
    $e = Get-Content $evidence -Raw | ConvertFrom-Json
    Write-Host ''
    Write-Host '=== the batch 6 census, re-read from the file the dispatch run wrote'
    ...
    Write-Host ''
    Write-Host '  STOPPING. No batch 6 evidence summary exists.'
    exit 2
}
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'shpw-door.log' -Label 'the execution door, at rest again' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'shpw-partition.log' -Label 'the partition and its installation' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'shpw-runner.log' -Label 'the runner invariants, unchanged' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'shpw-preflight.log' -Label 'the preflight gates, unchanged' | Out-Host
...
if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'shpw-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host '  NOTHING WAS CONSUMED. The authorisation stands at consumed 6, remaining 0 - the authorisation is fully spent.'
Write-Host '  THERE IS NO BATCH 7. The partition is closed and the ceiling is reached.'
