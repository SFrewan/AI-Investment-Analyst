...
<#
    SEC BATCH 5. ONE COMPANY. ONE REQUEST. ONE AUTHORISATION UNIT.
    ...
    SCOPE, and nothing wider:
      ...
      * ONE company: CIK 0001775625, SDC.US, SmileDirectClub, the single member of batch 5.
      * ONE request: submissions/CIK0001775625.json. The connector declares supportsWindow=false,
      ...
      * Four units are already spent - QUMU, LGIQ, ONEM, NGM - so this run must move 4 -> 5, leaving 1.

    SDC takes the SEVENTEEN SHARED FORMS and nothing else. The secondary reading scope - 424B*,
    ...
    SDC is the first member with TWO breaches, and they are consecutive: 2023-09-29 -> 2023-10-02,
    a fall of 61.18 per cent (0.415 -> 0.1611), and 2023-10-02 -> 2023-10-03, a further 53.45 per
    cent (0.1611 -> 0.075). Both are class A. Their selection windows are 2023-09-24 .. 2023-10-10
    and 2023-09-27 .. 2023-10-11, which overlap; the union is 2023-09-24 .. 2023-10-11, and the
    earliest start - 2023-09-24 - is the date the filing history has to reach back to for a count
    from this document to mean anything.

    EODHD is switched OFF throughout. Nothing here runs batch 1, 2, 3, 4 or 6.
...
$ExpectedCorrelation = 'batch-4-a1-0001775625-RegulatoryFilings'
...
foreach ($m in $flags) {
    ...
    $expected = if ($index -eq 5) { 'true' } else { 'false' }
    ...
    if ($index -eq 5) {
        if ($cik -ne '0001775625') { Write-Host ("  FAIL  batch 5 names CIK {0}, not 0001775625." -f $cik); $bad = $true }
        if ($symbol -ne 'SDC.US') { Write-Host ("  FAIL  batch 5 names symbol {0}, not SDC.US." -f $symbol); $bad = $true }
        if ($prior -ne 4) { Write-Host ("  FAIL  batch 5 declares prior consumption {0}, not 4." -f $prior); $bad = $true }
    }
}

if (-not $bad) {
    Write-Host '  PASS  1. batch 5 is SDC.US / CIK 0001775625'
    Write-Host '  PASS  2. batch 5 ExpectedPriorConsumption = 4'
    Write-Host '  PASS  4. Authorised is true for batch 5'
    Write-Host '  PASS  5. batches 1, 2, 3, 4, 6 are false'
}
...
if ($consumed -ne 4) {
    Write-Host ("  FAIL  3. actual consumed is {0}, not 4." -f $consumed)
    $bad = $true
}
else { Write-Host '  PASS  3. actual consumed = 4' }

if ($artefacts | Where-Object { $_.Name -like 'acquisition-sec-05-*' }) {
    Write-Host '  FAIL  7. a batch 5 artefact already exists.'
    $bad = $true
}
else { Write-Host '  PASS  7. no batch 5 artefact exists' }
...
Write-Host ''
Write-Host '  ================================================================'
Write-Host '   SEC BATCH 5 - SDC.US - CIK 0001775625'
Write-Host '   ONE request to data.sec.gov. ONE authorisation unit, the fifth.'
Write-Host '   submissions/CIK0001775625.json, RegulatoryFilings, NO window.'
Write-Host '   Seventeen shared forms; the secondary scope belongs to LGIQ only.'
Write-Host '   Ceiling 6, enforced and not amended. Batches 1-4 and 6 do NOT run.'
Write-Host '  ================================================================'
Write-Host ''
...
try {
    ...
    & powershell -NoProfile -ExecutionPolicy Bypass `
        ...
        -LogName 'sec-sdc.log' `
        -Label 'SEC batch 5: SDC, exactly one EDGAR filing request' | Out-Host
...
$artefact = Join-Path $root 'artifacts\universe\acquisition-sec-05-attempt-01.json'

Write-Host ''
Write-Host '=== batch 5 outcome'
...
try {
    ...
    # Every filing that falls inside the union of SDC's two selection windows, whatever its form.
    # This is what
    # lets "no scoped filing" be told apart from "no filing at all".
    $inWindow = @($rows | Where-Object { $_.filingDate -ge '2023-09-24' -and $_.filingDate -le '2023-10-11' })
    ...
    $summary = [pscustomobject]@{
        ...
        SelectionWindow             = '2023-09-24..2023-10-10 and 2023-09-27..2023-10-11 (union 2023-09-24..2023-10-11)'
    ...
    $out = Join-Path $root 'artifacts\verify\sec-sdc-evidence.json'
    ...
    Write-Host ('  coverage test: earliest <= 2023-09-24 ? ' + ($summary.EarliestFilingDate -le '2023-09-24'))
    ...
    Write-Host ('  ANY filing inside the window union 2023-09-24..2023-10-11 (any form): ' + $inWindow.Count)
...
Write-Host '  NO NEXT BATCH RAN. Batch 6 is unauthorised and needs its own approval.'
Write-Host '  BATCHES 1-4 DID NOT RUN AGAIN. Their flags are down and their units are spent.'
Write-Host '  THE CEILING WAS NOT AMENDED. The declaration and its digest are unchanged.'
Write-Host '  THE BATCH 5 FLAG MUST NOW BE RETURNED TO FALSE BEFORE THE FULL SUITE IS RUN.'
