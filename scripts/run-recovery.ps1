...
function Clear-Switches {
    ...
    Remove-Item Env:\AIINV_FULL_RECOVERY -ErrorAction SilentlyContinue
...
Write-Host '  SEC ceiling: 214 free requests, enforced by the transport.'
Write-Host '  This can take several minutes: one filing per unpriceable member.'

$code = 0
$recoveryReport = Join-Path $root 'artifacts\verify\full-survivorship-recovery.md'
...
if (Test-Path $recoveryReport) {
    ...
    try {
        $env:AIINV_FULL_RECOVERY = '1'

        & powershell -NoProfile -ExecutionPolicy Bypass `
            -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
            -Filter 'FullyQualifiedName~FullSurvivorshipRecoveryTests' `
            -LogName 'full-recovery.log' `
            -Label 'full historical identity recovery, at most 214 free SEC requests' | Out-Host
