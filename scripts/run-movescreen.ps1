...
<#
    SCREEN THE ELEVEN RECOVERED PRICE SERIES THROUGH THE PRODUCTION SPLIT ADJUSTER. READ-ONLY.
    ...
    artifacts\verify\gate6-recovered-series-move-screen.md
...
try {
    $env:AIINV_MOVE_SCREEN = '1'

    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~RecoveredSeriesMoveScreenTests' `
        -LogName 'move-screen.log' `
        -Label 'the eleven recovered series, screened by the production split adjuster' | Out-Host

    $code = [int]$LASTEXITCODE
}
finally { Remove-Item Env:\AIINV_MOVE_SCREEN -ErrorAction SilentlyContinue }

if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~AI.Investment' `
        -LogName 'movescreen-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host ''
Write-Host '=== Move screen outcome'

$report = Join-Path $root 'artifacts\verify\gate6-recovered-series-move-screen.md'

if (Test-Path $report) {
    Get-Content $report |
        Where-Object { $_ -match '^\| `|^\| \*\*|Recovered series screened|Gate 6 passes' } |
        Select-Object -First 60 |
        ForEach-Object { Write-Host ('  ' + ($_ -replace '[`*]', ' ')) }
}
else { Write-Host '  no move-screen report written.' }
