...
<#
    CHARACTERISE EVERY REFUSED SESSION PAIR AGAINST THE ARCHIVED ROWS AROUND IT. READ-ONLY.
...
try {
    $env:AIINV_MOVE_CHARACTERISATION = '1'

    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~UnexplainedMoveCharacterisationTests' `
        -LogName 'characterise.log' `
        -Label 'every refused session pair, read back against the archived rows around it' | Out-Host

    $code = [int]$LASTEXITCODE
}
finally { Remove-Item Env:\AIINV_MOVE_CHARACTERISATION -ErrorAction SilentlyContinue }

if ($code -eq 0) {
    & powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'gate-tests.ps1') `
        -Filter 'FullyQualifiedName~AI.Investment' `
        -LogName 'characterise-full.log' -Label 'full Release suite, connectors off' | Out-Host
...
Write-Host ''
Write-Host '=== Characterisation outcome'

$report = Join-Path $root 'artifacts\verify\gate6-unexplained-move-characterisation.md'

if (Test-Path $report) {
    Get-Content $report |
        Where-Object { $_ -match '^\| `|^\| \*\*|Breach pairs characterised|persistent level shift|immediate or repeated' } |
        Select-Object -First 60 |
        ForEach-Object { Write-Host ('  ' + ($_ -replace '[`*]', ' ')) }
}
else { Write-Host '  no characterisation report written.' }
