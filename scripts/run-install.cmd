@echo off
setlocal
rem PROVE THE INSTALLED SEC EDGAR AUTHORISATION LOADS - READ-ONLY. NO ACQUISITION.
...
rem   One report is written: artifacts\verify\sec-edgar-six-member-install-report.md

cd /d "%~dp0.."

echo.
echo ===============================================================
echo  SEC EDGAR INSTALL VALIDATION - READ-ONLY, NO ACQUISITION
echo   1. Release build, warnings are errors
echo   2. focused installation tests
...
dotnet build "%~dp0..\AI-Investment-Analyst.sln" -c Release --nologo > "%~dp0..\artifacts\verify\install-build.log" 2>&1
set EXITCODE=%ERRORLEVEL%
if not "%EXITCODE%"=="0" (
  echo   FAIL  Release build
  findstr /C:"error " "%~dp0..\artifacts\verify\install-build.log"
  goto :done
)
echo   PASS  Release build
echo.

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-install.ps1"
set EXITCODE=%ERRORLEVEL%
